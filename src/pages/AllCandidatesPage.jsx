...
import { Loader2, Users, Briefcase, TrendingUp, Filter, Search } from 'lucide-react';
...
const AllCandidatesPage = () => {
  ...
  // Count candidates with scores
  const scoredCount = sortedCandidates.filter(c => c.matchResult).length;
  const avgScore = scoredCount > 0 
    ? Math.round(sortedCandidates.reduce((sum, c) => sum + (c.matchResult?.overallScore || 0), 0) / scoredCount)
    : 0;
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-bold text-gray-900">{scoredCount > 0 ? `${avgScore}%` : '—'}</p>
        ...
        <>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-3 mb-4 flex flex-wrap items-center gap-3">
            ...
            <span className="text-sm text-gray-600">
              Showing all {candidates.length} candidates from {jobs.length} job{jobs.length > 1 ? 's' : ''}
            </span>
          </div>
